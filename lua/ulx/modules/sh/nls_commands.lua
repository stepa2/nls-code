...
local function Main()
    ...
    local cmd_rp = ulx.command("NLS", "ulx rp", CreateGamemodeCommand(NLS.Gamemodes.Types.RP), {"!rp"})
    cmd_rp:defaultAccess(ULib.ACCESS_ALL)
    cmd_rp:help("Переводит игрока в ролевой режим")
...
if NLS == nil then
    hook.Add("NLS.AfterInit", "NLS.Concommands", Main)
else
    Main()
end